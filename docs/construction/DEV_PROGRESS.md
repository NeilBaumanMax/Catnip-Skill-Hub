...
- 本轮先提交并 push 部署计划，创建远端备份 `backup/pre-tencent-neils-skill-hub-deploy-20260911-1048`；随后从提交归档构建 `linux/amd64` app 镜像，绝不把工作区未提交工具/依赖改动带入生产。
- 服务器先只读核验 current、Compose 服务、端口、磁盘、现有恢复点和旧 `catnip-intro`；写入前新增 PostgreSQL custom dump、SeaweedFS 对象归档、manifest 与 SHA-256，保留当前 release 和 app 镜像标签。
- 发布到独立 `/opt/catnip-skill-hub/releases/<commit>`，原子更新 `current`，只用既有 `/etc/catnip-skill-hub/env` 和 `--no-build` 重建 app/Caddy；不修改数据库内容、nginx、SSH、UFW、安全组、DNS、HTTPS、管理员凭据或旧工作区。
- 完成后验证四项长期服务、迁移状态、近期日志、回环与公网健康/首页/真实详情/推荐、三项下载、图片、响应式截图、公网管理 404、回环 18080 和公网 22/80 边界；失败则立即恢复旧 current 与旧 app 镜像并复测。

## 2026-09-11 10:12 CST / 原瀑布流恢复与管理员登录说明 / 最终 Git 回写

- 实现与验收提交 `ec4a674` 已成功 push 到 `origin/redesign/neils-skill-hub-colorful`；开发前备份 `backup/pre-waterfall-restore-20260911-0928` 保持可用。
- 本条为直接后继的远端状态记录；生产未部署，用户未提交文件继续隔离。

## 2026-09-11 09:40 CST / 原瀑布流恢复与管理员登录说明 / 完成记录

- 已撤销 Skill 区的 1+2 主次网格和首卡跨栏，恢复原有 4/3/2/1 列 CSS Grid 瀑布流；三种 size span 机制、DOM 顺序、多色卡片和约 25% 的紧凑尺度继续保留。
- 59/59、lint 0 error/3 个既有 warning、typecheck、db:check、Webpack build、diff check 与布局检测通过；标准 16 张截图及 Skill 区补充截图读图通过，390/768/1024/1440/1968px 断点与零溢出实测通过。
- 截图验收：通过（自动验收）。钥匙串项目只读确认存在；本机 18443 SSH 隧道当前未运行，管理员需先启动隧道再访问 `http://localhost:18443/admin/login`。未读取密码或修改生产。
...
- 开发前备份 `backup/pre-neils-scale-refine-20260911-0839` 保持可用；生产未部署，用户未提交文件保持隔离。

## 2026-09-11 09:09 CST / 公共页面尺度收紧与灵感兔替换 / 完成记录

- 已将宽屏容器、首页舞台、标题、搜索台、Skill 卡片、详情首屏和推荐表单整体收紧约 20%-30%；保持 44px 触控目标，不使用会降低可读性的整站缩放。
- 首页已使用新生成的无文字透明灵感兔插画 `public/brand/neil-rabbit-inspiration.png`，旧头像式 `neil-rabbit-mascot.png` 保留但不再渲染。
- 59/59、lint、typecheck、db:check、Webpack build、diff check 和 Impeccable layout 扫描通过；390/768/1968px 三公共路由无溢出、破图或控制台错误。
- 修改后 16 张标准截图及真实详情补充截图读图通过；基线局域网地址超时、移动顶栏溢出、Playwright 参数错误和 768px 卡片压扁均已记录、修复并复测。
- 截图验收：通过（自动验收）。生产未部署，用户既有未提交文件继续隔离。

## 2026-09-11 08:39 CST / 公共页面尺度收紧与灵感兔替换 / 开工计划

- Neil Bauman 认为当前首页舞台、顶栏和 Skill 卡片整体体量过大，明确允许约 25% 收紧；同时要求首页不再使用头像原图，改用其提供的“找灵感”兔子插画或无文字的同类生成图。
- 本轮在现有 `redesign/neils-skill-hub-colorful` 视觉世界内做尺度蒸馏：保持信息架构、色彩角色、路由与业务功能，优先减少容器最大宽度、舞台高度、标题字号、媒体高度、卡片间距和顶栏占用，让首屏更快进入 Skill 内容。
- 使用内置 ImageGen 参考用户第三张图生成无文字、无底部标牌、透明背景的灵感兔角色；保留白兔、圆框眼镜、星星眼、红色外套/围巾、举灯泡与书本动作，不直接把带中文字的原图放入页面。
- 修改 `src/app` 前生成三公共页面四视口基线截图；完成后运行 59 项测试、lint、typecheck、db:check、Webpack production build、HTTP、四视口截图与读图，检查 390px 溢出和触控目标。
- 开工计划提交并 push 后创建远端备份 `backup/pre-neils-scale-refine-20260911-0839`；不修改生产环境，不暂存既有用户文件。
...
- 网站将删除十个 Phase 2 演示 seed、`project-brief` 本地下载夹具及相应旧测试假设，改为用三个真实资源覆盖目录、搜索、推荐、安装与 Release 下载测试；不触碰用户未提交的截图工具、使用说明和依赖改动。
- 生产写入前建立完整 PostgreSQL/对象恢复点并验证校验和；精确删除十个演示 slug，发布干净提交，确认首页只剩三项、旧详情为 404、真实详情/下载/安装保持正常、公网管理继续 404。
- 内容主库同步移除首批十个示例资源，提升 catalog 版本并发布新的不可变 Release；历史 `v0.1.0`/`v0.2.0` 和 Git 记录保留，不移动或覆盖旧 Tag。
- 网站开发前备份计划为 `backup/pre-remove-demo-skills-20260816-0359`；内容主库使用独立同名备份。两仓计划提交和备份远端核验成功后才修改真实内容或服务器。
...
- 经 Neil Bauman 明确确认，生成新的随机强密码，先写入并读回验证 macOS 钥匙串，再仅把新 scrypt 哈希写入服务器受限环境文件；不在终端、聊天、Git、文档或服务器输出明文。
- 更新前保存服务器环境文件权限受限备份；更新后保持 `root:root 0600`，重建 app 并验证健康、SSH 隧道真实登录、授权 API、退出和旧密码失效。
- 当前工作区既存 `.gitignore`、`AGENTS.md`、README、依赖及截图工具改动继续隔离，不暂存、不覆盖；先提交本计划，创建并远端核验唯一备份 `backup/pre-admin-password-reset-20260807-1711`，再进行生产写操作。
...
- 远端开发前备份 `backup/pre-server-deployment-readiness-20260807` 保持指向计划提交 `349d4e8`。
- 本条为实现提交的直接后继状态回写；服务器仍为零写操作，下一门禁仍是腾讯云系统盘快照确认。
...
- 本条为直接后继纯文档状态回写，提交后再次 push；后续服务器写操作仍需新授权。
...
- 公共 header 现在是顶部唯一的滚动自适应毛玻璃环境层；内部品牌/搜索行不再有背景、四边框、圆角、阴影或第二次 blur。
- 分类与标签行以单条 hairline 和留白组织；标签入口默认透明，只在 hover、focus 或展开时反馈。
- 详情和推荐页路径上下文取消独立玻璃卡片，保留连续公共外壳和清晰 breadcrumb。
- 搜索输入组、按钮和标签下拉继续保留真实交互边界；搜索、推荐、分类单选、标签多选与 URL 行为未改变。

### 验证摘要

- `npm test` 57/57、lint、typecheck、db:check、生产 build、HTTP 与 `git diff --check` 最终通过。
- build 首次因受管沙箱禁止 Turbopack 内部进程绑定端口而失败；在获准环境执行同一命令后通过，失败与复测均写入 LOG。
- 修改后 12/12 标准截图逐张检查；桌面/手机标签展开态和桌面滚动态专项截图通过，无框中框、文字穿透、横向溢出或布局断裂。
- 自动截图通过不等于 Neil Bauman 已主观验收。
