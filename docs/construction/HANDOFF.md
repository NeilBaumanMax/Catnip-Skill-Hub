...
- 后续从当前分支接力时保留用户未提交文件；内容主库继续以 `v0.4.0` 提供知乎不可变 ZIP。
...
- 管理地址仍为 `http://localhost:18443/admin/login`，管理员标识仍为 `neil@catnipent.local`。公网 `http://118.195.247.102/admin` 继续由 nginx 返回 404，不得改用公网 HTTP 登录。
- 旧完成记录声称存在的钥匙串项目在本轮开始时实测不存在，原密码无法从服务器不可逆哈希恢复。Neil Bauman 明确授权后已重置密码；当前登录钥匙串服务 `Catnip Skill Hub Admin` 已真实验证项目存在且密码可读回，本轮完成时新密码也在 Neil 的剪贴板中。
- 若本地 `18443` 隧道中断，执行 `ssh -fNT -i /Users/neil/.ssh/catnipent -o IdentitiesOnly=yes -o ExitOnForwardFailure=yes -L 127.0.0.1:18443:127.0.0.1:18080 ubuntu@118.195.247.102`，再打开本地管理地址。

### 生产与恢复点

- 服务器代码发布仍为 `a578bca`；只更新仓库外 `/etc/catnip-skill-hub/env` 中的管理员 scrypt 哈希并重建 app/Caddy。环境文件仍为 `root:root 0600`，服务器没有密码明文。
- 密码重置前环境备份为 `/etc/catnip-skill-hub/env.pre-admin-password-reset-20260807-171455`，包含生产秘密，只允许 root 使用，不得复制到聊天或仓库。
- Git 开工计划 `158ee5d` 已 push；远端备份 `backup/pre-admin-password-reset-20260807-1711` 已核验指向同一提交。当前工作区用户工具改动继续隔离。

### 已验证与下一步

- 钥匙串读回、Compose config、容器内 scrypt 格式、四项长期服务健康和近期无错误日志通过；隧道登录页/新密码登录/授权/退出为 200，错误密码与退出后匿名访问为 401；公共健康 200，公网管理入口 404。
- 下一步 Neil 可直接粘贴剪贴板密码登录并录入 Skill。若以后再次声称凭据已保存，必须先执行不输出明文的钥匙串存在性和读回验证，不能只依据文档记录。
- 域名、HTTPS、MFA、登录审计/告警、UFW/安全组、系统更新、异机自动备份和监控仍未完成，本轮没有扩大这些范围。

## 2026-08-07 16:55 CST / 无域名管理员私网入口权威交接

- 实现与交接提交 `b6d2eee` 已成功 push 到当前远端分支；本条最终状态回写提交后再次 push。服务器发布保持 `a578bca`。
...
- 当前仍停在准备分支，服务器未连接、未部署；下一步必须等待 Neil Bauman 明确授权具体只读审计或写施工。
...
- 下一次 Codex 默认从 `main` 开工；`UI_fix` 只保留历史，不再承接开发。
- 主工作区的既有未提交工具改动仍需原样保护；服务器部署仍暂停。

## 2026-08-03 20:18 CST / main 主分支权威交接

> 本条覆盖下方所有把 `UI_fix` 写成当前分支的历史记录。Neil Bauman 已明确要求合并到主分支并在主分支继续开发。

### 当前状态

- 当前开发分支为 `main`；`UI_fix` 已通过快进方式完整推广，无冲突、无 force push、无历史删除。
- 推广前计划与安全基线为 `b3697c586b2a08f36ccf4f5a1e4583203e993018`；远端备份 `backup/pre-main-ui-fix-merge-20260803-1954` 已 push 并核验。
- `UI_fix`、`SKill-hub-ui`、`frontend/visual-optimization` 与 `backend-server-deployment` 仅保留为历史/隔离分支；下一轮不得误切回作为默认开发分支。
- 服务器部署继续暂停；没有新的服务器写授权。

### 当前公共前台与验证

- 工具栏保留唯一正式图形 Logo，公共 header 为文字品牌和单层毛玻璃；分类单选、标签下拉多选和重复 `tag` AND 语义保持不变。
- 首页、详情和推荐共享公共外壳；下载/安装仍只在详情页，Skill 仍是主角。
- 本轮 `npm test` 57/57，lint、typecheck、db:check、生产 build、diff check 全部通过；四个 HTTP 入口均为 `200`。
- 修改前与修改后均生成 12 张截图；修改后重点读图通过。自动截图不等同 Neil Bauman 对未来修改的主观验收。

### 工作区保护

主工作区在本轮开始前已有以下未提交用户工具改动，合并全程通过隔离 worktree 保护；下一轮仍不得删除、覆盖或顺手提交：`.gitignore`、`AGENTS.md`、`next-env.d.ts`、`package.json`、`package-lock.json`、`.agents/`、`scripts/screenshots.ts`、`skills-lock.json`。

### 下一轮第一步

1. 从 `main` 读取本条、核验 Git 和上述未提交文件；收到具体开发任务后再写开工计划并创建新的远端备份。
2. 可见前端修改继续执行修改前/后 3 页面 x 4 视口截图、读图、57 项测试基线及完整工程门禁。
3. 不自行恢复服务器部署，不合并或删除历史分支，不运行破坏性 Git 命令。

### 风险

- 本轮隔离 `npm ci` 报告 4 moderate 与 4 high 依赖审计项；未自动修改锁文件。后续若治理，必须单独立项、备份并验证。
- `globals.css` 的历史令牌/重复覆盖债务仍未处理，不应混入普通功能任务。
- 收尾文档提交与远端 `main` 最终 SHA 在 push 后追加回写。
...
- 最新实现提交：`6b0b786ce09d72f0cd57fea10a0f4240c8c58b12`
- 已 push：计划、备份分支和实现提交均已 push；本状态回写为直接后继纯文档提交
- 工作区状态：本轮实现已提交；既有用户工具改动隔离，因此全局工作区不是干净状态
...
- 最新实现提交：`263222f62ae19000309d6b61706f08449663b299`
- 已 push：计划、备份分支和实现提交均已 push；本状态回写为直接后继纯文档提交
- 工作区状态：本轮实现已提交；既有用户工具改动隔离，因此全局工作区不是干净状态
...
- 最新实现提交：`25b6e879c0e9e8fd3b55a01d8e28588179095fbc`
- 已 push：计划、备份分支和实现提交均已 push；本状态回写为直接后继纯文档提交
