...
Phase 0 使用 Next.js 16 App Router、React 19、TypeScript、Tailwind CSS 4、ESLint 9 与 npm。应用采用 `src/` 目录；公开路由位于 `src/app`。当前只包含最小占位首页，不接数据库或外部服务。
...
Phase 0 不为未实现层创建空代码目录；上述路径在对应 Phase 出现真实职责时创建。
