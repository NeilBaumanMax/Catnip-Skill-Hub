...
- `src/app/public-web.css` 是只在 `.public-site` 下生效的公共视觉系统；管理员页面继续使用既有 `globals.css`，本轮不改变管理端组件或业务流程。
- 首页、Skill 详情和推荐页显式挂载 `.public-site`，共享浅深色 token、系统中文字体、响应式网格、焦点、reduced-motion 与 reduced-transparency 降级。
- `src/app/_components/skill-artwork.tsx` 只负责把领域 `CoverTheme` 映射到版本化演示图片；不读取数据库、不生成图片、不决定下载或安装行为。
- `public/skill-art` 保存可替换的 Catnip 自制演示封面，只用于资源内容表达，不属于 `public/brand`，也不代表正式 Logo、吉祥物或真实效果证明。
- 安装命令、ZIP、统计、搜索和推荐提交继续经过原有领域或 API 边界；React 页面未接管这些服务职责。
