...
## 2026-08-07 19:02 CST / 三个真实 Skill 下载与安装扩展完成

- Idea、Apple、Dashi 三项下载均固定到内容主库 `v0.2.0`，允许 catalog Tag 与单包版本不同，但精确文件名、固定仓库和 SemVer Tag 门禁不变。
- 三个生产下载 API 均返回精确 307；远端 ZIP 已实际下载、SHA-256 与解压通过，并用 `skills@1.5.22` 在隔离项目安装到 Codex 成功。
- 三张生产详情页均展示可复制 Agent 命令和“下载 ZIP”，Dashi 约 25.6 MB 的完整分发保留许可边界，不单独抽取专有组件。
...
## 2026-07-31 19:32 CST / Phase 3 / 运行态复验

- 当前开发预览已从旧回环进程切换为 `http://192.168.110.9:3000`。
- 首页、详情页和 Release 下载重定向均实测成功。
- 12 张自动截图生成成功并完成重点读图，未发现布局断裂。
- 本轮只改变临时进程状态，无代码改动；下一阶段才评估其余 9 个内容资源的网站录入。
