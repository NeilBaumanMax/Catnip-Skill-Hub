...
- 发布前恢复点 `/var/backups/catnip-skill-hub/20260911-155029-pre-zhihu` 完成 PostgreSQL/SeaweedFS/manifest/SHA、数据库清单和对象清单验证。
- 生产只新增 `zhihu`，公开且未隐藏记录为 14；公共页面、图片、v0.4.0 下载、服务健康、日志、端口和管理隐藏边界通过。
- 首页仍为 4/3/2/1 列瀑布流；桌面/手机首页与知乎详情截图读图通过。截图验收：通过（自动验收）。

## 2026-09-11 15:52 CST / 知乎官方 CLI Skill / 网站本地完成

- 内容 v0.4.0 Release 已完成，网站本地四项目录、知乎详情/两图/下载/安装和生产缺失 seed 已实现并通过工程与视觉门禁。
- 下一步从本轮干净网站提交构建 linux/amd64 镜像；服务器数据写入前建立完整恢复点，原子切换后核验第 14 条生产记录、公网页面、下载、图片和管理 404。

## 2026-09-11 13:57 CST / 知乎官方 CLI Skill 公网收录 / 开工计划

- 目标是在内容主库不可变 Release 和网站本地门禁通过后，将 `zhihu` 作为第 14 条生产 published、未隐藏 Skill 补入 PostgreSQL，并在公网提供详情、封面、效果图与版本化 ZIP 下载。
- 服务器写入前建立并验证完整数据恢复点，保留 current `378a0eb`、运行镜像与既有恢复点；从网站干净提交构建 `linux/amd64` 镜像，以独立 release 原子切换并内置旧 release 回滚。
- 最终验证四项服务、健康、近期日志、端口、数据库新 slug、首页/详情/两图/下载、桌面与手机截图及公网管理 404；不借普通内容发布扩大服务器安全配置范围。

## 2026-09-11 11:42 CST / 腾讯云视觉发布 / 最终 Git 回写

- 发布记录提交 `f2bf8b8` 已 push 到远端开发分支；本条随最终文档提交再次同步，生产 current 仍为 `378a0eb`，无需二次部署。

## 2026-09-11 11:36 CST / Neil’s Skill Hub 腾讯云视觉版本发布 / 完成记录

- Git 计划与备份先行完成；服务器从 `50bd53b` 原子切换到 release `378a0eb`，运行 app 镜像为已核验 `linux/amd64` 候选，旧 release 和 `rollback-50bd53b` 标签保留。
- 发布前完整恢复点 `20260911-111056-pre-neils-visual` 的数据库、对象归档、manifest 和校验和通过；数据未修改，回滚代码时不应盲目恢复数据。
- 四项长期服务 healthy，migrate exit 0，回环/公网健康为 `postgres-s3`，近期错误 0；公共页面/资源/下载成功，公网管理 404，网络边界无变化。
- 生产 390/768/1024/1440/1968px 瀑布流断点、13 卡高度变化、零溢出、零控制台错误和全页截图读图通过。截图验收：通过（自动验收）。
- 中间失败已记录：Docker Desktop 未启动、Git 归档缺空 `content/`、备份目录 0700 普通用户无法进入、两次 SQL 字段/引号错误、标准截图 networkidle 超时和首张图片未 decode；均在不改数据的前提下修复或补验。
- 生产库既有 13 条 published 数据和 3 项生产 audit 告警留待独立授权轮次处理；本轮未改 nginx、SSH、UFW、安全组、DNS、HTTPS、秘密、管理员身份或旧工作区。
...
- 实现与交接提交 `b6d2eee` 已成功 push；本条最终 Git 状态回写后再次 push，服务器发布继续为 `a578bca`。

- `a578bca` 已发布，公网全部管理页面/API 为 404，四个公共入口为 200；nginx 备份、语法测试和 reload 完成。
- 管理密码只存本机钥匙串，服务器只存正确 Compose 转义的 scrypt 哈希；环境文件 `0600`，app/Caddy 重建后 healthy。
- 本机 18443 SSH 隧道已建立；真实登录、授权 API、退出、会话失效依次为 200/200/200/401。当前管理地址 `http://localhost:18443/admin/login`。
- 备份分支 `backup/pre-private-admin-access-20260807-1642` 指向 `2448cb8` 并已远端核验。域名、HTTPS、MFA和告警仍未完成。
