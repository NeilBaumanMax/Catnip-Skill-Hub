...
本文是 `frontend/visual-optimization` 分支的公共前台替换式重设计契约。Neil Bauman 已授权推翻旧前端方案并改为 Apple 风格 UI，后续最新指令又授权自主连续施工到设计完成并打开预览。因此：

- 当前状态：Stage C 至 G 的生产公共前台实现已落地，正在执行测试、漂移和 Git 收尾；Stage A 的自动化截图证据因浏览器运行时无实例而保持未完成。
- 当前授权：完成公共前台、全量验证、文档闭环、分支 push，并在局域网打开热更新预览后停止。
- 本轮禁止：安装无关依赖、生成正式品牌资产、改变后端或服务器。
- 本次连续施工共用一个开发前远端备份和完整闭环；未取得的浏览器证据必须保留为风险，不得用源代码检查代替。
...
- 根设计契约：`DESIGN.md`；已按真实 `src/app/public-web.css` token 和已落地组件完成扫描式固化。
