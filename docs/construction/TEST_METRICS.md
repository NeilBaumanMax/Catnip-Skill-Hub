...
- `PRODUCT.md`、`DESIGN.md` 和 `docs/construction/FRONTEND_REDESIGN_PLAN.md` 必须与实际实现一致；seed `DESIGN.md` 在真实 token 建立后改用扫描模式固化，不得长期把计划值写成实现事实。
- 首次代码施工前必须取得首页、至少一个 Skill 详情和推荐表单的桌面与移动截图基线；浏览器不可用时停止在文档/环境修复，不得声称视觉审计通过。
- 每个可见 Stage 至少检查 1440px、768px、390px，浅色/深色、键盘焦点、200% 缩放、长标题、图片失败和触摸设备无 hover 状态。
- 必须验证 `prefers-reduced-motion`；使用透明材质时还要验证减少透明度或实体背景降级。空间动画必须可中断，频繁键盘操作不得被动画延迟。
- 首页必须快速出现 Skill 内容，不使用大型搜索 Hero；卡片保持整卡链接且无直接下载；案例不成为首页主线。
- 公共视觉作用域不得污染管理员后台。任何 `globals.css` 重构都要对 `/admin/login` 和至少一个受保护管理页做回归或通过样式隔离证明无影响。
- 记录改造前后 Core Web Vitals 或等价性能证据；不得以“更像 Apple”或“更顺滑”代替性能、可访问性和浏览器证据。
- 新 UI、图标或动效依赖必须先经过必要性、React/Next 兼容性、包体、维护和 reduced-motion 评估；默认优先 CSS、View Transitions 或 WAAPI。
