...
- 当前生产发布来自 `redesign/neils-skill-hub-colorful`，服务器发布提交为 `8c1c340`；前一 release `378a0eb` 与其 app 回滚镜像保留，历史分支不删除。当前 UI 基线为 `Neil’s Skill Hub` 多彩紧凑主题、白兔 Logo/灵感插画、工具栏唯一图形 Logo、顶部文字品牌、分类单选、标签下拉多选和 4/3/2/1 列瀑布流。
